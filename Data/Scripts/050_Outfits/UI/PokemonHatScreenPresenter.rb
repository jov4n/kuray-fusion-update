class PokemonHatPresenter
  ...
  def getPicturePath()
    if @pokemon.isTripleFusion?
      picturePath = GameData::Species::getSpecialSpriteName(getDexNumberForSpecies(@pokemon.species))
    elsif @pokemon.isFusion?
      picturePath = get_fusion_sprite_path(@pokemon.species_data.head_pokemon.id, @pokemon.species_data.body_pokemon.id)
    else
      picturePath = get_unfused_sprite_path(@pokemon.species)
    end
    echoln picturePath
    return picturePath
  end
