module Compiler
  ...
  def compile_pokemon(path = "PBS/pokemon.txt")
    ...
    if File.exist?("migration_map.json")
      File.readlines("migration_map.json").each do |line|
        if line =~ /"(\d+)"\s*:\s*"([^"]+)"/
          legacy_species_map[$2.upcase] = $1.to_i
        end
      end
    end
    sections = []
    File.open(path, "rb") { |f|
      pbEachFileSectionEx(f) { |contents, section_name|
        sections.push([contents, section_name])
      }
    }
    ...
    sections.each do |contents, section_name|
      if section_name.nil? || section_name.to_s.strip.empty?
        raise _INTL("A Pokémon section must have a non-empty ID ({1}).", path)
      end
      if section_name.to_s.match?(/\A\d+\z/)
        raise _INTL("Numeric section IDs are no longer supported. Use <ID>_<AUTHOR> instead ({1}).", section_name)
      end
      species_id = GameData::SpeciesId.normalize(section_name)
      if !species_id.to_s.include?("_")
        legacy_number = legacy_species_map[species_id.to_s.upcase]
        if legacy_number
          species_id = "#{legacy_number}_#{GameData::SpeciesId::DEFAULT_AUTHOR}".to_sym
        elsif species_id.to_s.upcase.start_with?("FAKEMON")
          # Reserve a namespace bucket for legacy Fakemon sections.
          match = species_id.to_s.upcase.match(/\AFAKEMON(\d+)?\z/)
          fakemon_number = (match && match[1]) ? match[1].to_i : 0
          species_id = "#{fakemon_number}_FAKEMON".to_sym
        elsif species_id.to_s.match?(/\A\d+\z/)
          species_id = "#{species_id}_#{GameData::SpeciesId::DEFAULT_AUTHOR}".to_sym
        else
          raise _INTL("Species ID '{1}' must be in the form <NUMBER>_<AUTHOR>.", species_id)
        end
      end
      GameData::SpeciesId.namespace_and_key(species_id)
      local_number = GameData::SpeciesId.local_number(species_id)
      if local_number.nil?
        raise _INTL("Species ID '{1}' must be in the form <NUMBER>_<AUTHOR>.", species_id)
      end
      FileLineData.setSection(species_id, "header", nil)   # For error reporting
      # Raise an error if a species ID is used twice
      if GameData::Species::DATA[species_id]
        ...
        for key in schema.keys
          ...
          # Allow colon-prefixed symbols in PBS (e.g., :PSYCHIC) for enum fields
          if contents[key].is_a?(String) && schema[key] && schema[key][1].is_a?(String) && schema[key][1].include?("e")
            contents[key] = contents[key].gsub(/:([A-Z0-9_]+)/, '\1')
          end
          ...
          internal_name = GameData::SpeciesId.normalize(contents["InternalName"])
          if internal_name != species_id
            # Legacy internal names are auto-mapped to the section's canonical ID.
            internal_name = species_id
          end
          if GameData::Species::DATA[internal_name]
            raise _INTL("Species ID '{1}' is used twice.\r\n{2}", internal_name, FileLineData.linereport)
          end
          # Compile value for key
          value = pbGetCsvRecord(contents[key], key, schema[key])
          value = nil if value.is_a?(Array) && value.length == 0
        ...
        species_symbol = species_id
        species_hash = {
          :id                    => species_symbol,
          :id_number             => local_number,
          :name                  => contents["Name"],
        ...
        species_names[species_symbol]           = species_hash[:name]
        species_form_names[species_symbol]      = species_hash[:form_name]
        species_categories[species_symbol]      = species_hash[:category]
        species_pokedex_entries[species_symbol] = species_hash[:pokedex_entry]
    end
    # Enumerate all evolution species and parameters (this couldn't be done earlier)
    GameData::Species.each do |species|
      FileLineData.setSection(species.id, "Evolutions", nil)   # For error reporting
      Graphics.update if species.id_number % 200 == 0
      pbSetWindowText(_INTL("Processing {1} evolution line {2}", FileLineData.file, species.id)) if species.id_number % 50 == 0
  ...
  def compile_pokemon_forms(path = "PBS/pokemonforms.txt")
    ...
    form_number = 0
    GameData::Species.each do |species|
      next if !species.id_number || species.id_number < 0
