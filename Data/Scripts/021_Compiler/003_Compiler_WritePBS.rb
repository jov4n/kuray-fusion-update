module Compiler
  ...
  #=============================================================================
  def write_phone
    return if !safeExists?("Data/phone.dat")
  ...
  def write_pokemon
    ...
    File.open("PBS/pokemon.txt", "wb") { |f|
      ...
      GameData::Species.each do |species|
        ...
        if seen_ids[species.id]
          dup_ids << species.id
          next
        end
        seen_ids[species.id] = true
    ...
    if dup_ids.length > 0
      File.open("PBS/pokemon_dupes.txt", "wb") { |f|
        dup_ids.uniq.each { |id| f.write("#{id}\r\n") }
      }
    end
    pbSetWindowText(nil)
    Graphics.update
  ...
  def write_shadow_movesets
    return if !safeExists?("Data/shadow_movesets.dat")
      ...
      dex_lists.each_with_index do |list, index|
        ...
        list.each do |species|
          ...
          if current_family && current_family.include?(species)
            ...
            begin
              current_family = species_data.get_related_species
            rescue
              current_family = [species_data.id]
            end
            comma = false
            f.write("\r\n")
          end
          f.write(species_data.id.to_s)
          comma = true
        end
        f.write("\r\n")
      end
  ...
  def write_trainer_lists
    return if !safeExists?("Data/trainer_lists.dat")
  ...
  def write_all
    writers = [
      [:write_town_map, nil],
      [:write_connections, nil],
      [:write_phone, nil],
      [:write_types, nil],
      [:write_abilities, nil],
      [:write_moves, nil],
      [:write_items, nil],
      [:write_berry_plants, nil],
      [:write_pokemon, nil],
      [:write_pokemon_forms, nil],
      [:write_shadow_movesets, nil],
      [:write_regional_dexes, nil],
      [:write_ribbons, nil],
      [:write_encounters, nil],
      [:write_trainer_types, nil],
      [:write_trainers, nil],
      [:write_trainer_lists, nil],
      [:write_metadata, nil]
    ]
    errors = []
    writers.each do |method_name, _|
      begin
        self.send(method_name)
      rescue
        errors << "#{method_name}: #{$!.class} - #{$!.message}"
      end
    end
    if errors.length > 0
      File.open("PBS/write_all_errors.txt", "wb") { |f| f.write(errors.join("\r\n")) }
    end
  end
end
