...
module Compiler
  ...
  def compile_all(mustCompile)
    ...
    # Load existing data so we have dependencies for items/pokemon if their PBS are missing
    # We use a rescue here in case some .dat files are missing/corrupt
    begin
      GameData.load_all
    rescue Exception
      echoln "Warning: Could not load all existing data before compilation."
    end

    if File.exists?("PBS/townmap.txt")
      yield(_INTL("Compiling town map data"))
      compile_town_map
    end
    if File.exists?("PBS/connections.txt")
      yield(_INTL("Compiling map connection data"))
      compile_connections
    end
    if File.exists?("PBS/phone.txt")
      yield(_INTL("Compiling phone data"))
      compile_phone
    end
    if File.exists?("PBS/types.txt")
    ...
    if File.exists?("PBS/trainertypes.txt")
    ...
    if File.exists?("PBS/trainerlists.txt")
      yield(_INTL("Compiling battle Trainer data"))
      compile_trainer_lists
    end
    ...
    # System.reload_cache
    echoln ""
    echoln _INTL("*** Finished surgical compile ***")
    echoln ""
    pbSetWindowText(nil)
  end
  ...
  def main
    ...
    # Disabled bulk deletion to prevent data loss when PBS files are missing.
    # Each compile_ method will overwrite its own .dat file.
    if mustCompile
    end
    # Recompile all data
    begin
      ...
      pbPrintException(e)
      # Surgical fix: Do NOT delete data files on failure.
