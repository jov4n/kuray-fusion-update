...
def pbPrepareBattle(battle)
  ...
  if !backdrop
    map_meta = GameData::MapMetadata.try_get($game_map.map_id)
    isOutdoor = map_meta ? map_meta.outdoor_map : false
...
def pbWildBattleCore(*args)
  ...
  for arg in args
    if arg.is_a?(Pokemon)
      ...
      species_obj = GameData::Species.try_get(arg[0])
      species = species_obj ? species_obj.id : GameData::SpeciesId.normalize(arg[0])
      pkmn = pbGenerateWildPokemon(species,arg[1])
      foeParty.push(pkmn)
    elsif sp
      species_obj = GameData::Species.try_get(sp)
      species = species_obj ? species_obj.id : GameData::SpeciesId.normalize(sp)
