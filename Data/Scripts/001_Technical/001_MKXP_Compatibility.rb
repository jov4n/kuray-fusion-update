...
# Non-fusion custom sprite getter
def kurayGetCustomNonFusion(species, usedefault=0)
  dex_number = species.is_a?(Integer) ? species : GameData::Species.get(species)&.id_number
  ...
  usinglocation = "Graphics/BaseSprites/"
  if $POTENTIALSPRITES[dex_number].empty?
    # Support numeric filenames and Symbol filenames
    files = Dir[usinglocation + dex_number.to_s + "*.png"]
    if species.is_a?(Symbol)
      files += Dir[usinglocation + species.to_s + "*.png"]
    end
    files.uniq.each do |filename|
      next if filename == '.' or filename == '..'
      next if !filename.end_with?(".png")
      next if filename.end_with?("_i.png")
      filefile = File.basename(filename)
      dexname = filefile.split('.png')[0]
      if (dexname.to_s == dex_number.to_s || (species.is_a?(Symbol) && dexname.to_s == species.to_s)) && !$DEFAULTSPRITES.has_key?(dex_number.to_s)
...
def kurayGetCustomSprite(species, usedefault=0)
  return nil if species == nil
  
  if species.is_a?(Integer)
    # Handle legacy logic for numeric IDs
    if species <= Settings::NB_POKEMON
      return kurayGetCustomNonFusion(species, usedefault)
    elsif species >= Settings::ZAPMOLCUNO_NB
      return nil
    else
      # Check if this integer might actually be a registered base mon (new logic)
      if GameData::Species.exists?(species) && !species_is_fusion(species)
        return kurayGetCustomNonFusion(species, usedefault)
      end
      # Otherwise it's a legacy double fusion
      body_id = getBodyID(species)
      head_id = getHeadID(species, body_id)
      return kurayGetCustomDoubleFusion(species, head_id, body_id, usedefault)
    end
  else
    # Handle Symbol/String IDs
    if species_is_fusion(species)
      body_id = get_body_id_from_symbol(species)
      head_id = get_head_id_from_symbol(species)
      dex_number = get_dex_number(species) rescue 0
      return kurayGetCustomDoubleFusion(dex_number, head_id, body_id, usedefault)
    else
      return kurayGetCustomNonFusion(species, usedefault)
    end
  end
