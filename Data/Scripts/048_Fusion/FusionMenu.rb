class FusionSelectOptionsScene < PokemonOption_Scene
  ...
  def getAbilityName(ability)
    return "None" if ability.nil?
    ability_id = ability.respond_to?(:id) ? ability.id : ability
    return GameData::Ability.get(ability_id).real_name
  end

  def getAbilityDescription(ability)
    return "" if ability.nil?
    ability_id = ability.respond_to?(:id) ? ability.id : ability
    return GameData::Ability.get(ability_id).real_description
  end
