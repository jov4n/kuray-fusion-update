module GameData
  class FusedSpecies < GameData::Species
    ...
    def initialize(id)
      id = GameData::SpeciesId.normalize(id)
      ...
      @body_pokemon = GameData::Species.try_get(body_id)
      @head_pokemon = GameData::Species.try_get(head_id)

      if !@body_pokemon || !@head_pokemon
         # Fallback to a valid base pokemon to prevent total failure if data is missing
         # Log to console instead of popup
         body_label = body_id
         head_label = head_id
         if body_id
           body_ns, _body_key = GameData::SpeciesId.namespace_and_key(body_id)
           if GameData::SpeciesId.official_namespace?(body_ns)
             body_label = GameData::SpeciesId.local_number(body_id) || body_id
           end
         end
         if head_id
           head_ns, _head_key = GameData::SpeciesId.namespace_and_key(head_id)
           if GameData::SpeciesId.official_namespace?(head_ns)
             head_label = GameData::SpeciesId.local_number(head_id) || head_id
           end
         end
         echoln "Warning: Fusion #{id} has missing component: body=#{body_label} (#{@body_pokemon}), head=#{head_label} (#{@head_pokemon})" if $DEBUG
         fallback = GameData::Species.try_get(:PIKACHU) ||
                    GameData::Species.try_get(GameData::SpeciesId.official_id(25)) ||
                    GameData::Species::DATA.values.first
         @body_pokemon ||= fallback
         @head_pokemon ||= fallback
      end

      @id = GameData::SpeciesId.build_fusion(head_id, body_id)
      # For backward compatibility, fusions will still try to have an id_number
      # but we should move away from relying on it for logic.
    ...
    def get_body_id_generic(id)
      head_id, body_id = GameData::SpeciesId.split_fusion(id)
      return body_id
    end

    def get_head_id_generic(id)
      head_id, body_id = GameData::SpeciesId.split_fusion(id)
      return head_id
    end
    ...
    def calculate_dex_number()
      return 0 if !@body_pokemon || !@head_pokemon
      
      # KIF MIGRATION: If we are dealing with custom mons (virtual ID 0 or high), return a high virtual fusion ID
      if @body_pokemon.id_number <= 0 || @head_pokemon.id_number <= 0 || @body_pokemon.id_number > Settings::NB_POKEMON
         return Settings::KURAY_CUSTOM_POKEMONS + 1 # Simple virtual ID
      end

      # Legacy compat for standard mons
      begin
    ...
    def calculate_name()
      return "???" if !@body_pokemon || !@head_pokemon
      
      # KIF MIGRATION: Safe name calculation for Fakemon
      body_id_num = @body_pokemon.id_number
      head_id_num = @head_pokemon.id_number
      body_ns = @body_pokemon.respond_to?(:namespace) ? @body_pokemon.namespace : GameData::SpeciesId::DEFAULT_AUTHOR
      head_ns = @head_pokemon.respond_to?(:namespace) ? @head_pokemon.namespace : GameData::SpeciesId::DEFAULT_AUTHOR
      
      # If one of them is non-official or lacks split data, use a simpler naming convention
      if !GameData::SpeciesId.official_namespace?(body_ns) || !GameData::SpeciesId.official_namespace?(head_ns) ||
         body_id_num <= 0 || head_id_num <= 0 ||
         !GameData::SPLIT_NAMES[body_id_num] || !GameData::SPLIT_NAMES[head_id_num]
        # Basic symbolic name construction
        head_name = @head_pokemon.name[0..(@head_pokemon.name.length/2).to_i]
        body_name = @body_pokemon.name[(@body_pokemon.name.length/2).to_i..-1]
        return head_name + body_name
      end

      body_nat_dex = GameData::NAT_DEX_MAPPING[body_id_num] ? GameData::NAT_DEX_MAPPING[body_id_num] : body_id_num
      head_nat_dex = GameData::NAT_DEX_MAPPING[head_id_num] ? GameData::NAT_DEX_MAPPING[head_id_num] : head_id_num
      begin
    ...
    def calculate_evolutions()
      ...
      for evolution in body_evolutions
        ...
        species_id = GameData::SpeciesId.normalize(evolutionSpecies)
        next if GameData::SpeciesId.fusion?(species_id)
        evolutionSpecies_data = GameData::Species::DATA[species_id]
        next if !evolutionSpecies_data
        
        # KIF MIGRATION: Use symbolic ID generation
        fused_species = get_fusion_symbol(@head_pokemon.id, evolutionSpecies_data.id)
      ...
      for evolution in head_evolutions
        ...
        # KIF MIGRATION: Use symbolic ID generation
        fused_species = get_fusion_symbol(evolutionSpecies_data.id, @body_pokemon.id)
        fused_evolutions << build_evolution_array(evolution, fused_species)
      end
    ...
    def get_random_dex_entry()
      begin
        ...
        body_entries = all_body_entries[@body_pokemon.id.to_s]
        head_entries = all_body_entries[@head_pokemon.id.to_s]
        raise if !body_entries || !head_entries || body_entries.empty? || head_entries.empty?

        body_entry = body_entries.sample
        ...
        head_entry = head_entries.sample
