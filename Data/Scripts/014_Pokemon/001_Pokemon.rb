...
class Pokemon
  ...
  def species_data
    @species = GameData::SpeciesId.normalize(@species)
    if @species_data && @species_data.species == @species
      return @species_data
    end
    @species_data = GameData::Species.try_get(@species)
    if !@species_data && $DEBUG
      echoln "Warning: Missing species data for #{@species.inspect} (raw=#{@species.class})"
    end
    return @species_data
  end
  ...
  # @param species_id [Symbol, String] id of the species to change this Pokémon to
  def species=(species_id)
    normalized_species = GameData::SpeciesId.normalize(species_id)
    new_species_data = GameData::Species.try_get(normalized_species)
    return if !new_species_data
  ...
  def isFusion?
    ...
    return false if species_data.generation == 99
  ...
  def isTripleFusion?
    ...
    return dex_num >= Settings::ZAPMOLCUNO_NB
  end
  ...
  def growth_rate
    return GameData::GrowthRate.get(species_data.growth_rate) if species_data
    return GameData::GrowthRate.get(:Medium)
  end
  ...
  def baseStats
    ...
    if !species
      ret = {}
      GameData::Stat.each_main { |s| ret[s.id] = 1 }
      return ret
    end
    if $PokemonSystem.custom_bst > 0 && species.is_a?(GameData::FusedSpecies) && player_owned?
      this_base_stats = species.calculate_base_stats_custom($PokemonSystem.custom_bst, $PokemonSystem.custom_bst_sliders)
    elsif $PokemonSystem.custom_bst_npc > 0 && species.is_a?(GameData::FusedSpecies) && !player_owned?
      this_base_stats = species.calculate_base_stats_custom($PokemonSystem.custom_bst_npc, $PokemonSystem.custom_bst_sliders_npc)
    else
      this_base_stats = species.base_stats
    end
    base_stats_exception = getBaseStatsFormException()
    this_base_stats = base_stats_exception if base_stats_exception
    ret = {}
    GameData::Stat.each_main { |s| ret[s.id] = this_base_stats[s.id] || 1 }
    return ret
  end
  ...
  def initialize(species, level, owner = $Trainer, withMoves = true, recheck_form = true)
    normalized_species = GameData::SpeciesId.normalize(species)
    @species_data = GameData::Species.try_get(normalized_species) ||
                    GameData::Species.try_get(:PIKACHU) ||
                    GameData::Species::DATA.values.first
    @species = @species_data ? @species_data.species : normalized_species
    @form = @species_data ? @species_data.form : 0
    @forced_form = nil
    @time_form_set = nil
    self.level = level
    @steps_to_hatch = 0
    ...
    @kuraygender = rand(65536)
    #KurayX - Custom Filenames
    @kuraycustomfile = kurayGetCustomSprite(@species)
    @veryunique = createVeryUnique()
