...
class PokemonIconSprite < SpriteWrapper
  ...
  def get_customicons_sprite_path(head_id,body_id)
    ...
    head_num = getDexNumberForSpecies(head_id)
    body_num = getDexNumberForSpecies(body_id)
    filename = sprintf("%s.%s_i.png", head_num, body_num)
    local_custom_path = Settings::CUSTOM_BATTLERS_FOLDER_INDEXED + head_num.to_s + "/" +filename
  ...
  def createFusionIcon(pokemon)
    if !pokemon || !pokemon.species || pokemon.species == 0
      return
    end
    # echoln "Debug Fusion Icon: Species=#{pokemon.species} (#{pokemon.species.class})"
    ...
    body_id = GameData::SpeciesId.normalize(bodyPoke_number)
    head_id = GameData::SpeciesId.normalize(headPoke_number)
    body_species_data = GameData::Species.try_get(body_id)
    head_species_data = GameData::Species.try_get(head_id)

    if !body_species_data
       echoln "Warning: Body species not found for #{bodyPoke_number}"
       bodyPoke = :PIKACHU
    else
       bodyPoke = body_species_data.species
    end

    if !head_species_data
       echoln "Warning: Head species not found for #{headPoke_number}"
       headPoke = :PIKACHU
    else
       headPoke = head_species_data.species
    end
    ...
    if dexNum.is_a?(Symbol)
      dex_obj = GameData::Species.try_get(dexNum)
      dexNum = dex_obj ? dex_obj.id_number : 0
    end
