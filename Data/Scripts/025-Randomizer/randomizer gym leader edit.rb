...
def getCustomSpeciesList(allowOnline = true, redownload_file=false)
  ...
  if speciesList.length <= 20000 && allowOnline
    ...
    for file in online_list
      ...
      species_id_list << dexnum if dexnum
    end
...
def getDexNumFromFilename(filename)
  begin
    ...
    return nil if head <= 0 || body <= 0 || head > NB_POKEMON || body > NB_POKEMON
    head_id = GameData::SpeciesId.official_id(head)
    body_id = GameData::SpeciesId.official_id(body)
