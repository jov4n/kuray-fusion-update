...
class PokemonPokedexInfo_Scene
  ...
  def pbStartSceneBrief(species)
    ...
    @viewport = Viewport.new(0, 0, Graphics.width, Graphics.height)
    @viewport.z = 99999
    dexnum = GameData::SpeciesId.display_label(species)
    dexnumshift = false
  ...
  def drawPageInfo
    ...
    indexText = GameData::SpeciesId.display_label(@species)
    # end
    species_name = species_data ? species_data.name : "???"
    textpos = [
      [_INTL("{1}{2} {3}", indexText, " ", species_name),
    ...
    if $Trainer.owned?(@species)
      ...
      evolutions = species_data ? species_data.get_evolutions : []
      ...
      height = species_data ? species_data.height : 0
      weight = species_data ? species_data.weight : 0
  ...
  def   drawEntryText(overlay, species_data)
    ...
    if species_data && species_data.is_fusion
      ...
      entryText = species_data ? species_data.pokedex_entry : "No Pokédex entry available."
      shadowColor = shadow
    end
...
class PokemonPokedexInfoScreen
  ...
  def pbStartSceneSingle(species)
    ...
    dexnum = species_obj ? species_obj.id_number : 0 #pbGetRegionalNumber(region,species)
    dexnumshift = Settings::DEXES_WITH_OFFSETS.include?(region)
    species_name = species_obj ? species_obj.name : "???"
    dexlist = [[species, species_name, 0, 0, dexnum, dexnumshift]]
