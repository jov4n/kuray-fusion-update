...
class Window_Pokedex < Window_DrawableCommand
  ...
  def drawItem(index,_count,rect)
    ...
    indexNumber = @commands[index][4] || 0
    ...
    if $Trainer.seen?(species)
      ...
      label = GameData::SpeciesId.display_label(species)
      text = sprintf("%-12s %s", label, @commands[index][1])
    else
      text = sprintf("%-12s ----------", "???")
    end
...
=end
  ...
  def pbGetDexList(filter_owned=false)
    ...
    GameData::Species.each do |species|
      ...
      dedupe_key = species_id
      if species.respond_to?(:id_number) && species.id_number &&
         GameData::SpeciesId.official_namespace?(species.namespace)
        dedupe_key = "OFFICIAL_#{species.id_number}"
      end
      next if seen_ids[dedupe_key]
      next if !$Trainer.seen?(species_id)
      next if filter_owned && !$Trainer.owned?(species_id)
      dex_label = GameData::SpeciesId.display_label(species_id)
      dexlist.push([species_id, species.real_name, species.height, species.weight, dex_label, false])
  ...
  def pbRefreshDexList(index=0)
    ...
    when MODENUMERICAL
      ...
      dexlist.compact!
      # Sort species in ascending order by symbolic label
      dexlist.sort! { |a,b| a[4].to_s <=> b[4].to_s }
    when MODEATOZ
      dexlist.sort! { |a,b| (a[1]==b[1]) ? a[4].to_s <=> b[4].to_s : a[1]<=>b[1] }
    when MODEHEAVIEST
      dexlist.sort! { |a,b| (a[3]==b[3]) ? a[4].to_s <=> b[4].to_s : b[3]<=>a[3] }
    when MODELIGHTEST
      dexlist.sort! { |a,b| (a[3]==b[3]) ? a[4].to_s <=> b[4].to_s : a[3]<=>b[3] }
    when MODETALLEST
      dexlist.sort! { |a,b| (a[2]==b[2]) ? a[4].to_s <=> b[4].to_s : b[2]<=>a[2] }
    when MODESMALLEST
      dexlist.sort! { |a,b| (a[2]==b[2]) ? a[4].to_s <=> b[4].to_s : a[2]<=>b[2] }
    end
    @dexlist = dexlist
  ...
  def pbRefresh
    ...
    if iconspecies
      species_obj = GameData::Species.try_get(iconspecies)
      textpos.push([species_obj ? species_obj.name : "???",112,46,2,base,shadow])
    end
    if @searchResults
