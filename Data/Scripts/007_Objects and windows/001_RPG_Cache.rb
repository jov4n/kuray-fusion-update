    ...
    def self.load_bitmap(folder_name, filename, hue = 0, rcode=0, gcode=1, bcode=2, pifshiny=0, kifshiny=0)
      ...
      if !ret
        if filename == ""
          ...
          fallback_path = nil
          if path =~ /Graphics\/Battlers\/(\d+)_0\/\1_0\.png$/i
            fallback_path = path.sub(/\/(\d+)_0\/\1_0\.png$/i, "/\\1/\\1.png")
          end
          if fallback_path && pbTryString(fallback_path)
            ret = BitmapWrapper.new(fallback_path)
          else
