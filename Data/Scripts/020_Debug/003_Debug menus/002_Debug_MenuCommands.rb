...
def pbDebugNormalizeSpecies(species)
  return nil if species.nil?
  raw = species
  if raw.is_a?(String) || raw.is_a?(Symbol)
    text = raw.to_s
    if (match = text.match(/\AB(\d+)H(\d+)\z/i))
      return GameData::SpeciesId.official_fusion_id(match[1].to_i, match[2].to_i)
    end
  end
  return GameData::SpeciesId.normalize(raw) rescue raw
end
...
DebugMenuCommands.register("testwildbattle", {
  ...
  "effect"      => proc {
    ...
    if species
      species = pbDebugNormalizeSpecies(species)
      species_data = GameData::Species.try_get(species)
      if !species_data
        pbMessage(_INTL("That species isn't available."))
        next false
      end
