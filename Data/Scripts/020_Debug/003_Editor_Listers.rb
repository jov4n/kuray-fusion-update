...
class TrainerBattleLister
  ...
  def commands
    ...
    for t in cmds
      trainer_type_data = GameData::TrainerType.try_get(t[1])
      trainer_type_name = trainer_type_data ? trainer_type_data.name : t[1].to_s
      if t[3] > 0
        @commands.push(_INTL("{1} {2} ({3}) x{4}",
           trainer_type_name, t[2], t[3],
           GameData::Trainer.get(t[1], t[2], t[3]).pokemon.length))
      else
        @commands.push(_INTL("{1} {2} x{3}",
           trainer_type_name, t[2],
           GameData::Trainer.get(t[1], t[2], t[3]).pokemon.length))
      end
    ...
    if !@includeNew || index > 0
      ...
      if tr_data
        tr_data.pokemon.each_with_index do |pkmn, i|
          ...
          species_name = species_data ? species_data.real_name : pkmn[:species].to_s
          text += sprintf("%s Lv.%d", species_name, pkmn[:level])
        end
      end
    end
    @pkmnList.text = text
