...
def download_custom_sprite(head_id, body_id, spriteformBody_suffix = "", spriteformHead_suffix = "", alt_letter="")
  ...
  head_data = GameData::Species.try_get(head_id) rescue nil
  body_data = GameData::Species.try_get(body_id) rescue nil
  use_numeric = head_data && body_data &&
                GameData::SpeciesId.official_namespace?(head_data.namespace) &&
                GameData::SpeciesId.official_namespace?(body_data.namespace)
  return nil if !use_numeric
  head_key = use_numeric ? (head_data.id_number || 0) : head_id.to_s
  body_key = use_numeric ? (body_data.id_number || 0) : body_id.to_s
  return nil if head_key.to_i <= 0 || body_key.to_i <= 0
  head_key = head_key.to_s + spriteformHead_suffix.to_s
  body_key = body_key.to_s + spriteformBody_suffix.to_s
  alt_letter = "" if !alt_letter.is_a?(String) || !alt_letter.match?(/\A[a-z]+\z/i)
  ...
  destPath = _INTL("{1}{2}", Settings::CUSTOM_BATTLERS_FOLDER_INDEXED, head_key)
  if !Dir.exist?(destPath)
    Dir.mkdir(destPath)
  end
  sprite = download_sprite(_INTL(url, head_key, body_key, alt_letter), head_key, body_key, destPath, alt_letter)
  return sprite if sprite
  return nil
end
