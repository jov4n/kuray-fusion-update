...
def getPokemon(dexNum)
  species_id = GameData::SpeciesId.normalize(dexNum)
  return GameData::Species.try_get(species_id)
end
...
def reverseFusionSpecies(species)
  return species if !species_is_fusion(species)
  dexId = getDexNumberForSpecies(species)
  return species if isTripleFusion?(dexId)
  body = getBasePokemonID(species, true)
  head = getBasePokemonID(species, false)
  newspecies = get_fusion_symbol(body, head)
