...
ItemHandlers::UseOnPokemon.add(:SLOWPOKETAIL, proc { |item, pokemon, scene|
  ...
  #next false if pokemon.species != :SHELLDER
    pbFadeOutInWithMusic(99999) {
      evo = PokemonEvolutionScene.new
      evo.pbStartScreen(pokemon, GameData::SpeciesId.official_fusion_id(90, 80))
      evo.pbEvolution(false)
      evo.pbEndScreen
  ...
  if !is_fusion
    if pokemon.fused != nil
      ...
      if chosen >= 0
        ...
        is_poke2_fusion = poke2.isFusion?
        
        if !is_poke2_fusion && poke2 != pokemon
          #check if fainted
...
def pbFuse(pokemon, poke2, splicer_item)
  ...
  # Pass Symbols to ensure Fakemon (id_number -1) are handled correctly
  newid = getSpeciesIdForFusion(poke2.species, pokemon.species)
  fus = PokemonFusionScene.new
