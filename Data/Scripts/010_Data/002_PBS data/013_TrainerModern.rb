  ...
  class TrainerModern
    ...
    def replaceFusionsHeadWithSpecies(species)
      ...
      if species_data.is_fusion
        ...
        return get_fusion_symbol(headPoke, bodyPoke)
      end
      return species
    end
    ...
    def to_trainer
      ...
      @pokemon.each do |pkmn_data|
        ...
        raw_species = pkmn_data[:species]
        if raw_species.is_a?(String) || raw_species.is_a?(Symbol)
          text = raw_species.to_s
          if (match = text.match(/\AB(\d+)H(\d+)\z/i))
            raw_species = GameData::SpeciesId.official_fusion_id(match[1].to_i, match[2].to_i)
          end
        end
        species_data = GameData::Species.try_get(raw_species)
        if !species_data
          fallback = (defined?(Settings::RIVAL_STARTER_PLACEHOLDER_SPECIES) ? Settings::RIVAL_STARTER_PLACEHOLDER_SPECIES : :MEW)
          echoln _INTL("Trainer data missing species {1}; using {2} instead.", pkmn_data[:species], fallback) rescue nil
          species_data = GameData::Species.try_get(fallback)
        end
        if !species_data
          echoln _INTL("Trainer data missing fallback species; skipping entry.") rescue nil
          next
        end
