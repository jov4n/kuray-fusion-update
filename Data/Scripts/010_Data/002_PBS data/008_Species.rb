module GameData
  class Species
    ...
    def initialize(hash)
      @id = hash[:id]
      @id_number = hash[:id_number] || GameData::SpeciesId.local_number(hash[:id]) || -1
      ...
      if hash[:namespace] || hash[:internal_key]
        @namespace = GameData::SpeciesId.normalize_author(hash[:namespace] || GameData::SpeciesId::DEFAULT_AUTHOR)
        @internal_key = hash[:internal_key] || @id.to_s
      else
        namespace, internal_key = GameData::SpeciesId.namespace_and_key(@id)
        @namespace = namespace
    ...
    def is_fusion
      # Check if self is explicitly a FusedSpecies class instance
      return true if self.is_a?(GameData::FusedSpecies)

      return GameData::SpeciesId.fusion?(@id)
    end
    #FUSION CALCULATIONS
    ...
    def get_body_species
      return @body_pokemon || self
    end

    def get_head_species
      return @head_pokemon || self
    end
    ...
    def apply_metrics_to_sprite(sprite, index, shadow = false)
      front_sprite_y = self.is_fusion ? get_body_species.front_sprite_y: @front_sprite_y
    ...
    def get_family_evolutions(exclude_invalid = true)
      ...
      evos = evos.sort do |a, b|
        a_data = GameData::Species.try_get(a[0])
        b_data = GameData::Species.try_get(b[0])
        (a_data ? a_data.id_number : 0) <=> (b_data ? b_data.id_number : 0)
      end
      ret = []
      evos.each do |evo|
        ret.push([@species].concat(evo)) # [Prevo species, evo species, method, parameter]
        evo_data = GameData::Species.try_get(evo[0])
        next if !evo_data
        evo_array = evo_data.get_family_evolutions(exclude_invalid)
    ...
    def get_baby_species(check_items = false, item1 = nil, item2 = nil)
      ...
      if ret != @species
        ret_data = GameData::Species.try_get(ret)
        return @species if !ret_data
        ret = ret_data.get_baby_species(check_items, item1, item2)
      end
      return ret
    end
    ...
    def get_related_species
      ...
      sp_data = GameData::Species.try_get(sp)
      return [sp] if !sp_data
      evos = sp_data.get_family_evolutions(false)
