class Sprite_Wearable < RPG::Sprite
  ...
  def crop_spritesheet(direction)
    ...
    bmp_w = @sprite.bitmap.width
    bmp_h = @sprite.bitmap.height
    return if bmp_w <= 0 || bmp_h <= 0
    frame_w = [@frameWidth, bmp_w].min
    frame_h = [@frameHeight, bmp_h].min
    return if frame_w <= 0 || frame_h <= 0
    sprite_x = 0
    sprite_y = ((direction - 2) / 2) * @frameHeight
    # Clamp to bitmap bounds to avoid invalid rectangles.
    sprite_y = [[sprite_y, 0].max, bmp_h - frame_h].min
    @sprite.src_rect.set(sprite_x, sprite_y, frame_w, frame_h)
  end
